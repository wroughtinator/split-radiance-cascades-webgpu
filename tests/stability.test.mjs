...
test("Algorithm 3 allocation, LOD overlap, and history are implemented", () => {
  ...
  assert.match(finalShader, /let visibilityGuardEnd=select\(\s*intervalEnd,intervalEnd\*15\.0,backFacingSurface\s*\)/);
...
test("reference and final-frame quality gates are compiled into production", () => {
  ...
  assert.match(computeShader, /resolvedPrimaryIrradiance\(\s*vec2i\(pixel\),world\.xyz,normal,fract\(world\.w\)>0\.25\s*\)/);
