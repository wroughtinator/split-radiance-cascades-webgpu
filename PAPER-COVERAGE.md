...
| --- | --- | --- |
| Eq. 1-2, Lambertian directional irradiance | 32 c0 equal-area directions are cosine-integrated into a bordered 6x6 octahedral irradiance field. | 128-spp cosine-weighted BVH reference and RGB error image. |
...
| Algorithm 1, sparse RC | Visible surfaces insert half-cell c0 probes; parent cascades are sparse hash maps; gathers renormalize only existing trilinear neighbors. | Ten-scene capacity/finite-value tests and live overflow counters. |
...
| Algorithm 3, hierarchical R2 | Counts propagate c0-to-c3, reverse offsets assign contiguous parent segments, and each ray receives an exact deterministic local rank plus one global temporal rotation. | Byte-identical independent trajectory replay. |
...
| Section 6, irradiance optimization | A 6x6 octahedral field with an evaluated one-texel border replaces the expensive per-pixel 32-direction gather. | WebGPU shader validation and reference comparison. |
...
sampling, interval, merge, LOD, temporal, or shading equations. BVH profiling
...
tests the paper field (exact-key probes), the deterministic renderer, and this
final presented image independently.
